...
static uint16_t prevPR = 0;
extern uint8_t Counters[PCS_N];
// ========== for CRC computation ========== 
...
void hookBlock(uc_engine *uc, uint64_t address, uint32_t size, void *user_data) {
    // use CRC16 to hash the basic block addr into some 16-bits number
    uint16_t pr = crc16(address);
    uint16_t idx = pr ^ prevPR;
    Counters[idx]++;
    // for debugging
    // printf("counters idx is %x, pr is %x, address is %p, prev PC is %x\n", idx, pr, address, prevPR);
    prevPR = (pr >> 1);
}
